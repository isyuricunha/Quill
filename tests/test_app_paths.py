...
class AppPathsTests(unittest.TestCase):
    ...
    def test_installed_data_lives_in_localappdata_and_migrates_legacy_files(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            install_dir = root / "Programs" / "Quill"
            runtime_root = install_dir / "_internal"
            legacy_data = runtime_root / "data"
            legacy_data.mkdir(parents=True)
            (legacy_data / "config.json").write_text('{"installed": true}', encoding="utf-8")
            ...
            self.assertEqual(data_dir, local_app_data / "Quill")
            self.assertEqual(
                (data_dir / "config.json").read_text(encoding="utf-8"),
                '{"installed": true}',
            )
